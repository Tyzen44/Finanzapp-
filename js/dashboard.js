            ...
            const accountCard = `
                <div class="account-card">
                    <div class="account-header">
                        ...
                        <button onclick="editAccountBalance('${account.id}')" class="action-btn edit">Ã¢Å“ÂÃ¯Â¸Â</button>
                    </div>
        ...
        const accountCard = `
            <div class="account-card" style="grid-column: span 3;">
                <div class="account-header">
                    ...
                    <button onclick="editAccountBalance()" class="action-btn edit">Ã¢Å“ÂÃ¯Â¸Â</button>
                </div>
                <div class="account-balance-hero">
                    CHF ${realTimeBalance.toLocaleString()}
                </div>
                <div class="account-details">
                    Aktueller Kontostand (inkl. VerfÃƒÂ¼gbar)
                </div>
            </div>
        `;
        dashboardGrid.insertAdjacentHTML('beforeend', accountCard);
...
function updateDashboardStats() {
    ...
    let totalExpenses = 0;
    let totalDebts = 0;
    let actualSavings = 0; // NEU: TatsÃƒÂ¤chliche Sparrate
    ...
    if (appData.currentProfile === 'sven') {
        ...
        // Berechne tatsÃƒÂ¤chliche Sparrate (Ausgaben mit Kategorie "Sparen")
    ...
    if (savingsRateElement) {
        // ANGEPASSTE ANZEIGE: Zeige tatsÃƒÂ¤chliche Sparrate UND Sparbetrag
        if (actualSavings > 0) {
        ...
        // Color coding fÃƒÂ¼r Sparrate basierend auf tatsÃƒÂ¤chlichen Werten
        if (savingsRate >= 20) {
...
function calculateAll() {
    ...
    if (trendElement) {
        if (available >= 0) {
            trendElement.textContent = `Ã°Å¸â€œË† +CHF ${available.toLocaleString()} monatlich`;
            trendElement.className = 'balance-trend text-success';
            if (availableStat) {
                availableStat.style.background = 'linear-gradient(135deg, #d4edda, #c3e6cb)';
            }
        } else {
            trendElement.textContent = `Ã°Å¸â€œâ€° CHF ${available.toLocaleString()} monatlich`;
...
function updateRecommendations() {
    ...
    if (overdueDebts.length > 0) {
        recommendations.push({
            type: 'danger',
            title: 'Ã¢Å¡ Ã¯Â¸Â ÃƒÅ“berfÃƒÂ¤llige Schulden',
            text: `${overdueDebts.length} Rechnung(en) im Wert von CHF ${overdueDebts.reduce((sum, d) => sum + d.amount, 0).toLocaleString()}.- sind ÃƒÂ¼berfÃƒÂ¤llig!`
        });
    }
    ...
    if (balance < 1000) {
        recommendations.push({
            type: 'danger',
            title: 'Ã°Å¸Å¡Â¨ Kritischer Kontostand',
        ...
        recommendations.push({
            type: 'warning',
            title: 'Ã¢Å¡ Ã¯Â¸Â Kontostand aufbauen',
            text: `Mit CHF ${balance.toLocaleString()}.- haben Sie eine Basis-Reserve. Ziel: CHF 5'000.- fÃƒÂ¼r Sicherheit.`
        });
    }
    ...
    if (available < 0) {
        recommendations.push({
            type: 'danger',
            title: 'Ã°Å¸Å¡Â¨ Budget-ÃƒÅ“berschreitung',
            text: `Sie ÃƒÂ¼berziehen um CHF ${Math.abs(available).toLocaleString()}.-. Reduzieren Sie dringend Ihre Ausgaben.`
        });
    } else if (available < 500 && appData.currentProfile !== 'family') {
        recommendations.push({
            type: 'warning',
            title: 'Ã¢Å¡ Ã¯Â¸Â Knappes Budget',
            text: `Nur CHF ${available.toLocaleString()}.- verfÃƒÂ¼gbar. Vorsicht bei zusÃƒÂ¤tzlichen Ausgaben!`
        });
    } else if (balance >= 5000 && available >= 1000) {
        recommendations.push({
            type: 'success',
            title: 'Ã¢Å“â€¦ Finanzen im grÃƒÂ¼nen Bereich',
            text: `Solider Kontostand und CHF ${available.toLocaleString()}.- verfÃƒÂ¼gbar. Perfekt fÃƒÂ¼r Investitionen!`
        });
    }
    ...
    if (totalDebts > income * 2 && income > 0) {
        recommendations.push({
            type: 'warning',
            title: 'Ã°Å¸â€œâ€¹ Hohe Schuldenlast',
        ...
        recommendations.push({
            type: 'info',
            title: 'Ã°Å¸â€œâ€¹ Schulden manageable',
    ...
    if (appData.currentProfile === 'family' && income === 0) {
        recommendations.push({
            type: 'warning',
            title: 'Ã°Å¸â€™Â¸ Keine ÃƒÅ“bertrÃƒÂ¤ge als Ausgaben erfasst',
            text: 'Gemeinschaftskonto hat keine aktiven ÃƒÅ“bertrag-Ausgaben. Erfassen Sie ÃƒÅ“bertrÃƒÂ¤ge als "ÃƒÅ“bertrÃƒÂ¤ge" Kategorie in den privaten Profilen.'
        });
    }
    ...
    if (appData.currentProfile !== 'family' && calculateTransfers()[appData.currentProfile === 'sven' ? 'fromSven' : 'fromFranzi'] === 0) {
        recommendations.push({
            type: 'info',
            title: 'Ã°Å¸â€™Â¡ ÃƒÅ“bertrÃƒÂ¤ge erstellen',
            text: 'Sie kÃƒÂ¶nnen Geld von Ihrem privaten Konto zum Gemeinschaftskonto ÃƒÂ¼bertragen.'
        });
    }
    ...
    if (recommendations.length === 0) {
        recommendations.push({
            type: 'success',
            title: 'Ã°Å¸Å½â€° Alles im grÃƒÂ¼nen Bereich',
