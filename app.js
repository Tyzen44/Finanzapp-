// ============= SWISS FINANCE V2.1 - ADDITIONAL INCOME FEATURE =============
...
class AppState {
    constructor() {
        this.data = {
            ...
            debts: [],
            additionalIncome: [], // NEW: Additional income entries
    ...
    // Get total additional income for current month
...
class SwissFinanceApp {
    ...
    renderIncome() {
                    ...
                    <div class="glass-card" style="margin-bottom: 24px;">
                                ...
                                <div class="expense-item">
                                    <div class="expense-header">
                                        <div class="expense-info">
                                            ...
                                            <div class="expense-category">${income.type} • ${new Date(income.date).toLocaleDateString('de-CH')}</div>
                                        </div>
                                        <div class="expense-amount">CHF ${income.amount.toLocaleString()}</div>
                                        <div class="expense-actions">
                                            <button class="action-btn delete" onclick="app.deleteAdditionalIncome(${income.id})">🗑️</button>
                                        </div>
                                    </div>
                                </div>
                            `).join('')
                        ...
                        ${totalAdditionalIncome > 0 ? `
                            <div class="total-card" style="background: linear-gradient(135deg, #43e97b, #38f9d7);">
                                <div class="total-amount">CHF ${totalAdditionalIncome.toLocaleString()}</div>
                    ...
                    ${transferExpenses.length > 0 ? `
                        ...
                        <div class="glass-card" style="margin-bottom: 24px;">
                            ...
                            <div class="total-card">
                                ...
                                <div class="total-label">Monatliche Transfers</div>
                            </div>
                        </div>
                    ` : ''}
        ...
        const totalExpenses = expenses.reduce((sum, e) => sum + e.amount, 0);
        const totalIncome = currentIncome + totalAdditionalIncome;
            ...
            <div class="tab-content active">
                ...
                <div class="glass-card" style="margin-bottom: 24px;">
                    ...
                    <div style="background: rgba(255,255,255,0.1); padding: 16px; border-radius: 8px; margin-bottom: 16px;">
                        <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 16px;">
                            <div>
                                <div style="font-size: 12px; opacity: 0.9;">Gehalt</div>
                                <div style="font-size: 18px; font-weight: 600;">CHF ${currentIncome.toLocaleString()}</div>
                            </div>
                            <div>
                                <div style="font-size: 12px; opacity: 0.9;">Zusätzlich</div>
                                <div style="font-size: 18px; font-weight: 600;">CHF ${totalAdditionalIncome.toLocaleString()}</div>
                            </div>
                            <div>
                                <div style="font-size: 12px; opacity: 0.9;">Verfügbar</div>
                                <div style="font-size: 18px; font-weight: 600;">CHF ${available.toLocaleString()}</div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="glass-card" style="margin-bottom: 24px;">
                    ...
                    <div style="text-align: center; margin: 20px 0;">
                        ...
                        <div class="info-box info">
                            ...
                            3️⃣ Überschuss = (Gehalt + Zusatzeinnahmen) - Ausgaben<br>
                            4️⃣ Überschuss wird zu Ihrem <strong>Kontostand hinzugefügt</strong><br>
                            5️⃣ Variable Ausgaben werden gelöscht<br>
                            6️⃣ Säule 3a Ausgaben werden automatisch für nächsten Monat eingetragen<br>
                            7️⃣ Zusätzliche Einnahmen werden <strong>NICHT gelöscht</strong> (Historie bleibt erhalten)
                        </div>
                    </div>
                </div>
                                ...
                                <div class="expense-header">
                                    <div class="expense-info">
                                        ...
                                        <div class="expense-category">
                                            ...
                                            ${entry.additionalIncome ? ` | Zusatz: CHF ${entry.additionalIncome.toLocaleString()}` : ''}
                                        </div>
                                    </div>
            ...
            <div class="tab-content active">
                ...
                <div class="glass-card">
                    ...
                    <p style="font-size: 14px; line-height: 1.6;">
                        ...
                        <strong>Zusatzeinnahmen:</strong> ${this.state.data.additionalIncome.length}<br>
    ...
    addAdditionalIncome() {
        ...
        this.showModal(
            '✨ Zusätzliche Einnahme hinzufügen',
            `
                <div class="form-row">
                    <label class="form-label">Bezeichnung</label>
                    <input type="text" id="additional-income-name" class="form-input" placeholder="z.B. Jahresbonus 2025">
                </div>
                <div class="form-row">
                    <label class="form-label">Betrag (CHF)</label>
                    <input type="number" id="additional-income-amount" class="form-input" placeholder="0.00" step="0.01">
                </div>
                <div class="form-row">
                    <label class="form-label">Typ</label>
                    <select id="additional-income-type" class="form-input">
            ...
            `,
            [
                { label: '💾 Speichern', primary: true, action: 'app.saveAdditionalIncomeFromModal()' },
                { label: '↩ Abbrechen', action: 'app.closeModal()' }
            ]
        );
    }

    saveAdditionalIncomeFromModal() {
        ...
        this.closeModal();
        alert(`✅ Zusätzliche Einnahme von CHF ${amount.toLocaleString()} hinzugefügt!`);
    }
    ...
    closeMonth() {
        ...
        const confirmMsg = `📅 Monat abschließen?\n\n` +
            ...
            `➕ ─────────────\n` +
            `💵 Gesamt: CHF ${totalIncome.toLocaleString()}\n\n` +
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        await app.init();
        console.log('🇨🇭 Swiss Finance V2.1 ready!');
    } catch (error) {
